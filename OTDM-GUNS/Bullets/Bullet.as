...
void onInit(CBlob@ this)
{
    ...
    //play gun sound
	if(this.hasTag("shotgun"))
	{
		this.getSprite().PlaySound("/Shotgun.ogg");
	}
	if(!this.hasTag("shotgun") && !this.hasTag("revolver"))
	{
		this.getSprite().PlaySound("/AKFire.ogg");
	}
	if(this.hasTag("revolver"))
	{
		this.getSprite().PlaySound("/Revolver.ogg");
	}
...
void onTick(CBlob@ this)
{
	...
	CSprite@ sprite = this.getSprite();
	if (this.hasTag("revolver") && sprite !is null)
	{
		sprite.PlaySound("Revolver.ogg");
		this.Untag("revolver");
	}

	if (this.hasTag("ak") && sprite !is null)
	{
		this.getSprite().PlaySound("AKFire.ogg");
	...
	if (this.hasTag("shotgun") && sprite !is null)
	{
		this.getSprite().PlaySound("Shotgun.ogg");
		this.Untag("shotgun");
	}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			dmg = ArrowHitBlob(this, point1, initVelocity, dmg, blob, Hitters::crush, arrowType);
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		ArrowHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::crush);
	}
}
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
	if (hitBlob !is null)
	{
		...
		// check if shielded
		const bool hitShield = (hitBlob.hasTag("shielded") && blockAttack(hitBlob, velocity, 1.0f));
		...
		else
		{
			...
			{
				ArrowHitMap(this, worldPoint, velocity, damage, Hitters::crush);
			}
		}
	}
