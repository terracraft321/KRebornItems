...
void onInit(CBlob@ this)
{
	...
	this.Tag("projectile");
	this.SetLight(true);
	this.SetLightRadius(10.0f+(XORRandom(60.0f)));
	this.SetLightColor(SColor(255, 255, 121, 0));

	CSprite@ sprite = this.getSprite();

	sprite.SetFrameIndex(XORRandom(2));

	sprite.SetEmitSound("Noise.ogg");
	...
	//set a random frame
	{
		Animation@ anim = sprite.addAnimation("arrow", 4, true);
		anim.AddFrame(0);
		anim.AddFrame(1);
		anim.AddFrame(2);
		anim.AddFrame(3);
...
{	
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (this.hasTag("shotgunned"))
		{
			...
			{
				this.server_Hit(this, this.getPosition(), Vec2f(), 1.0f, Hitters::fire);
			}
		}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			dmg = ArrowHitBlob(this, point1, initVelocity, dmg, blob, Hitters::fire, arrowType);
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		ArrowHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::fire);
	}
}
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
	if (hitBlob !is null)
	{
		...
		else
		{
			...
			{
				//ArrowHitMap(this, worldPoint, velocity, damage, Hitters::fire);
			}
		}
	}
