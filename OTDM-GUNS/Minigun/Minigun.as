...
void onInit(CBlob@ this)
{
	...
	//holder.SetKeysToTake(key_action1 | key_action2);
...
void onTick(CBlob@ this)
{
	...
	//this.SetKeysToTake(key_action1 | key_action2);
	...
	if (pressed_a1 && firedtime <= 0 && ammunition > 0)
		...
		if (this !is null)
		{
			...
			if (bullet !is null && point !is null && holder !is null)
			{
				...
				if (point.isKeyPressed(key_down) && holder.isOnGround() /*&& holder.getVelocity() == Vec2f(0, 0)*/)
				{
					...
					Vec2f norm = aim - pos;
					norm.Normalize();
					bullet.setVelocity(norm * ((36.0f+(XORRandom(2.0f)))));
					holder.AddForce(norm *(-6.0f));	
				}
				else
				{			
					Vec2f aim = (holder.getAimPos()+Vec2f(-10+XORRandom(20), -10+XORRandom(20)));
					Vec2f norm = aim - pos;
					norm.Normalize();
					bullet.setVelocity(norm * (33.0f+(XORRandom(5.0f))));
					holder.AddForce(norm *(-68.0f));
				}
