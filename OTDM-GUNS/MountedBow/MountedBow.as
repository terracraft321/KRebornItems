...
void onInit(CBlob@ this)
{
	...
	if (getNet().isServer())
	{
		...
		CBlob@ ammo2 = server_CreateBlob("mat_arrows");
		CBlob@ ammo3 = server_CreateBlob("mat_arrows");
		if (ammo !is null)
		{
			...
			if (!this.server_PutInInventory(ammo2))
				ammo2.server_Die();
			if (!this.server_PutInInventory(ammo3))
				ammo3.server_Die();
		}
	}
}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		bullet.getShape().setDrag(bullet.getShape().getDrag() * 1.5f);
