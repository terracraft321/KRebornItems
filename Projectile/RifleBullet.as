...
const f32 ARROW_PUSH_FORCE = 10.0f;
...
{
	this.getSprite().PlaySound("Rifle.ogg");
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	//	ArrowHitMap(blob, blob, this.getOldVelocity(), 6.5f, Hitters::arrow);
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		ArrowHitMap(this, end, this.getOldVelocity(), 6.5f, Hitters::arrow);
	}
}
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
	if (hitBlob !is null)
	{
		...
		if(specialArrowHit(hitBlob))
		{
			...
			//die
			this.server_Hit(this, this.getPosition(), Vec2f(), 6.0f, Hitters::crush);
		}
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f)
		{
			...
			if (hitBlob.getHealth() < 1.5f)
			{
				hitBlob.server_Die();
			}*/
			hitBlob.server_SetHealth(hitBlob.getHealth()-1.0f);
			if (hitBlob.getHealth() < 1.5f)
			{
				hitBlob.Damage(2.0f, this);
			}
			hitBlob.AddForce(velocity * force);
			if (hitBlob.getHealth() < 0.5f)
