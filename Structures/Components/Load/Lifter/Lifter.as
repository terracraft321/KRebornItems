...
class Lifter : Component
{
		u16 id;
	f32 angle;
	Vec2f offset;

	Lifter(Vec2f position, u16 _id, f32 _angle, Vec2f _offset)
	{
		x = position.x;
		y = position.y;
...
void onSetStatic(CBlob@ this, const bool isStatic)
{
	...
	Lifter component(position, this.getNetworkID(), angle, offset);
...
//Vec2f velo = (0.0f, 0.0f);
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	//Vec2f velo = (blob.getVelocity());
	if(blob !is null)
	{
		if (this.getPosition().y > (blob.getPosition().y+(blob.getRadius())))
		{
			if (this.get_bool("is active")) 
			{
				this.set_bool("is colliding", true);
				blob.setVelocity(Vec2f((blob.getVelocity().x*1.15), -7));
				if (this.getTeamNum()==99)
				{
					blob.setVelocity(Vec2f((blob.getVelocity().x*1.15), -20));

				}
				//blob.AddForce(Vec2f((this.getVelocityX), -(blob.getMass()*4.5)));
