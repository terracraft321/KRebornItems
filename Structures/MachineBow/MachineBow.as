...
const Vec2f arm_offset = Vec2f(-6, -20);
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		bullet.server_SetTimeToDie(10.69f);
...
void onInit(CBlob@ this)
{
	...
	if (this.getTeamNum()==192)
	{	
		Vehicle_SetupWeapon(this, v,
	                    	5, // fire delay (ticks)
	                    	1, // fire bullets amount
	                    	Vec2f(-6.0f, 2.0f), // fire position offset
	                    	"boulder", // bullet ammo config name
	                    	"boulder", // bullet config name
	                    	"fallbig", // fire sound
	                    	"EmptyFire" // empty fire sound
	                 		);
	}
	...
	if (this.getTeamNum()==192)
	{
		this.set_string("autograb blob", "boulder");
